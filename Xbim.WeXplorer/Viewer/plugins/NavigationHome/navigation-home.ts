    ...
    */
    ...
    public init(xviewer: Viewer) {
        ...
        self._image.addEventListener("click",
            function() {
                let viewer = self._viewer;
                //set target to full extent
                viewer.setCameraTarget();
                let origin = viewer._origin;
                let distance = self.distance != null ? self.distance : viewer._distance;

                let normDirection = vec3.normalize(vec3.create(), self.viewDirection);
                let position = vec3.scale(vec3.create(), normDirection, -1.0 * distance);
    ...
    private _adjust() {
        let canvas = this._viewer._canvas;
