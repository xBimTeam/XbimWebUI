...
import { DrawMode, ModelHandle } from "./model-handle";
...
export class Viewer {
    ...
    */
    constructor(canvas: string | HTMLCanvasElement) {
        ...
        if (canvas instanceof HTMLCanvasElement) {
            ...
            this._canvas = document.getElementById(canvas as string) as HTMLCanvasElement;
        }
        ...
        */
        ...
        const gl = WebGLUtils.setupWebGL(this._canvas, { preserveDrawingBuffer: true });
    ...
    public renderingMode: RenderingMode = RenderingMode.NORMAL;
    ...
    */
    public static check() {
        ...
        */
        ...
        if (typeof Int32Array === "undefined" || typeof Float32Array === "undefined")
            result.errors
    ...
    */
    public defineStyle(index: number, colour: number[]) {
        if (typeof (index) == "undefined" || (index < 0 && index > 224)) throw new Error("Style index has to be defined as a number 0-224");
        if (typeof (colour) == "undefined" || !colour.length || colour.length != 4) throw new Error("Colour must be defined as an array of 4 bytes");
    ...
    */
    public setState(state: State, target: number | number[]) {
        if (typeof (state) == "undefined" || !(state >= 225 && state <= 255)) throw new Error("State has to be defined as 225 - 255. Use xState enum.");
    ...
    */
     ...
     */
    public getModelState(id: number): number[][] {
     ...
     */
    public restoreModelState(id: number, state: number[][]) {
        const handle = this._handles[id];
        if (typeof (handle) === "undefined") {
            throw new Error("Model doesn't exist");
        }
    ...
    */
    public setCameraPosition(coordinates: number[]) {
        if (typeof (coordinates) == "undefined") throw new Error("Parameter coordinates must be defined");
    ...
    */
    public setCameraTarget(prodId?: number): boolean {
        ...
        const setDistance = function(bBox: number[] | Float32Array) {
            const size = Math.sqrt(bBox[3] * bBox[3] + bBox[4] * bBox[4] + bBox[5] * bBox[5]);
    ...
    private getBiggestRegion(): Region {
        const volume = function(box: Float32Array) {
        ...
        const handle = this._handles
            .filter((h, i, a) => { !h.stopped; })
            .sort((a, b) => {
                const volA = volume(a.region.bbox);
                const volB = volume(b.region.bbox);
                if (volA < volB) {
                    return -1;
                }
                if (volA == volB) {
    ...
    */
    public loadAsync(loaderUrl: string, model: string | Blob | File, tag?: any): void {
        if (typeof (model) == "undefined") throw new Error("You have to specify model to load.");
        ...
        worker.onmessage = function(msg) {
            ...
            const geometry = msg.data as ModelGeometry;
    ...
    */
    ...
    private addHandle(geometry: ModelGeometry, tag?: any): void {
        ...
        const handle = new ModelHandle(viewer.gl, geometry);
        ...
        if (viewer._handles.length === 1) {
            ...
            //set default view
            viewer.setCameraTarget();
            const dist = Math.sqrt(viewer._distance * viewer._distance * 1.5);
         ...
         *
        */
        viewer.fire("loaded", { id: handle.id, tag });
    ...
    public unload(modelId: number) {
        const handle = this._handles.filter(function(h) { return h.id === modelId; }).pop();
        if (typeof (handle) === "undefined")
        ...
        handle.unload();
         ...
         /**
         * Occurs when geometry model is unloaded from the viewer. This event returns object containing ID of the model created in 'loaded' event.
         *
    ...
    private _initContextMenuEvent() {
        this._canvas.addEventListener("contextmenu", (event: MouseEvent) => {
            ...
            */
            this.fire("contextMenu", { event, id: this.getIdFromEvent(event) });
    ...
    private _initMouseEvents() {
        ...
        const handleMouseUp = (event: MouseEvent) => {
            ...
            if (deltaX < 3 && deltaY < 3 && button === "left") {
                ...
                * @param {Number} id - product ID of the element or null if there wasn't any product under mouse
                */
                if (!handled) viewer.fire("pick", { id, event });
            }
        ...
        * @param {Number} id - product ID of the element or null if there wasn't any product under mouse
        */
        this._canvas.addEventListener("dblclick", () => { viewer.fire("dblclick", { id }); }, true);
    ...
    private _initTouchNavigationEvents() {
        ...
        const handleTouchMove = (event: TouchEvent) => {
            ...
            if (event.touches.length === 1) {
                ...
                const deltaX = event.touches[0].clientX - lastTouchX_1;
                const deltaY = event.touches[0].clientY - lastTouchY_1;
                lastTouchX_1 = event.touches[0].clientX;
                lastTouchY_1 = event.touches[0].clientY;
    ...
    private _initTouchTapEvents() {
        ...
        const maximumLengthBetweenDoubleTaps = 200;
        ...
        const handleTouchStart = (event: TouchEvent) => {
            ...
            const viewX = lastTouchX - r.left;
            const viewY = this._height - (lastTouchY - r.top);

            //this is for picking
            id = this.getID(viewX, viewY);

            const now = new Date();
            const isDoubleTap = (now.getTime() - lastTap.getTime()) < maximumLengthBetweenDoubleTaps;
            if (isDoubleTap) {
                this.fire("dblclick", { id });
            }
            lastTap = now;
        ...
        const handleTouchEnd = (event: TouchEvent) => {
            ...
            const deltaX = Math.abs(endX - lastTouchX);
            const deltaY = Math.abs(endY - lastTouchY);
    ...
    private navigate(type, deltaX, deltaY) {
        ...
        const distance = Math.max(vec3.vectorLength(distanceVec), this._handles[0].meter);
    ...
    */
    ...
    private isChanged() {
        ...
        this._visualStateAttributes.forEach((vsa) => {
            const state = JSON.stringify(this[vsa]);
            const lastState = this._lastStates[vsa];
            this._lastStates[vsa] = state;
            theSame = theSame && (state === lastState);
        });

        const activeHandlesCount = this._handles.filter((h) => !h.stopped).length;
    ...
    */
    public show(type: string) {
        ...
        switch (type) {
            ...
            case "bottom":
                ...
                const toOrigin = mat4.translate(mat4.create(),
                    mat4.create(),
    ...
    public getIdFromEvent(event: MouseEvent): number {
        ...
        //get coordinates within canvas (with the right orientation)
        const r = this._canvas.getBoundingClientRect();
        const viewX = x - r.left;
        const viewY = this._height - (y - r.top);
    ...
    */
    public stopPicking(id: number) {
        const model = this._handles.filter((h) => h.id === id).pop();
        if (typeof (model) === "undefined")
            throw new Error("Model doesn't exist.");
    ...
    */
    public getClip(): number[][] {
        const cp = this._clippingPlaneA;
        if (!this._clippingA || cp.every((e) => e === 0)) {
            return [[0, 0, 0], [0, 0, 0]];
        }

        const normal = vec3.normalize(vec3.create(), [cp[0], cp[1], cp[2]]);
    ...
    */
    public clip(point?: number[], normal?: number[]) {
        ...
        const ns = "http://www.w3.org/2000/svg";
        const svg = this.getSVGOverlay();
        const viewer = this;
        const position: { x?: number, y?: number, angle?: number } = {};
        let down = false;
        let g: SVGElement;
        ...
        const handleMouseMove = function(event) {
            ...
            let angle = Math.atan2(dX, dY) * -180.0 / Math.PI + 90.0;
